...
export default function UsersPage() {
  ...
  // Form states
  const [createForm, setCreateForm] = useState({ email: "", name: "", role: "viewer" as const });
  const [editForm, setEditForm] = useState({ name: "", role: "viewer" as const });
  ...
  const RoleBadge = ({ role }: { role: string }) => {
    const config: Record<string, { icon: React.ReactNode; color: string }> = {
      admin: { icon: <Shield className="h-3 w-3" />, color: "bg-red-500 text-white" },
      developer: { icon: <UserCheck className="h-3 w-3" />, color: "bg-purple-500 text-white" },
      viewer: { icon: <Eye className="h-3 w-3" />, color: "bg-blue-500 text-white" },
    };
                  ...
                  <Button variant="outline">
                    ...
                    {hasActiveFilters && (
                      <Badge variant="secondary" className="ml-2 text-xs">
                        {[filters.search, filters.role, filters.isActive].filter(Boolean).length}
                      </Badge>
                    )}
                  </Button>
                </DialogTrigger>
                  ...
                  <div className="space-y-4 py-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setTempFilters({ ...tempFilters, role: v as any })}
                      >
                        <SelectTrigger>
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => setTempFilters({ ...tempFilters, isActive: v === "all" ? undefined : v === "true" })}
                      >
                        <SelectTrigger>
        ...
        {total > 0 && (
          <div className="flex items-center justify-between px-6 py-4 border-t bg-muted/30">
            <p className="text-sm text-muted-foreground">
              Showing {(filters.page - 1) * filters.limit + 1}–{Math.min(filters.page * filters.limit, total)} of {total} users
            </p>
