...
export default function ViewershipPage() {
  ...
  const [data, setData] = useState<ViewershipItem[]>([]);
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await eventsService.getViewership({
      ...
      toast.error("Failed to load viewership data");
      console.error(err);
      setData([]);
      setTotal(0);
    } finally {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const filename = `viewership_${filters.date}${
            ...
            <ButtonGroup>
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    <DialogTitle>Filter Viewership</DialogTitle>
                    <DialogDescription>
