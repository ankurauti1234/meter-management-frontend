...
export default function ButtonPressedReportPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await eventsService.getButtonPressedReport({
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      const headers = ["Device ID", "HHID", "Date", "Button Pressed"];

      const csvRows = [
        headers.join(","),
        ...exportData.map((item) =>
          [item.device_id, item.hhid, item.date, item.button_pressed].join(",")
        ),
      ];
      ...
      const filename = `button_pressed_${filters.date}.csv`;
  ...
  const columns: ColumnDef<ButtonPressedItem>[] = [
    {
    ...
    {
      accessorKey: "button_pressed",
      header: "Button Pressed",
      cell: ({ row }) => (
        <Badge
          variant={row.original.button_pressed === "Yes" ? "default" : "secondary"}
          className={
            row.original.button_pressed === "Yes"
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          // In declared_members column cell
          <div className="flex flex-wrap gap-1.5">
            {members.map((m) => (
              <span
                key={m.code}
                className={`text-sm font-medium px-1.5 py-0.5 rounded-md border ${
                  m.active
            ...
            <ButtonGroup>
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent className="max-w-2xl">
                  <DialogHeader>
                    <DialogTitle>Filter Button Pressed</DialogTitle>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-64 text-center">
                    ...
                    <p className="mt-2 text-muted-foreground">Loading button pressed data...</p>
                  </TableCell>
                </TableRow>
              ) : data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={columns.length} className="h-64 text-center">
                    <Empty>
                      <EmptyHeader>
                        <EmptyMedia variant="icon"><Fingerprint className="h-12 w-12 text-muted-foreground" /></EmptyMedia>
