...
export default function AlertsPage() {
  ...
  const hasActiveFilters = Boolean(
    filters.device_id || startDateTime.date || endDateTime.date
  );
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      ...
      const alerts: AlertEvent[] = res.events.map((e: Event) => ({
        ...e,
        details: e.data || {},
      }));
  ...
  const handleResetFilters = () => {
    const reset = { device_id: "", page: 1, limit: filters.limit };
    setFilters(reset);
  ...
  return (
    <div className="p-4 space-y-6">
      <PageHeader
        title="Alerts"
        description="Real-time critical alerts from all devices"
        badge={
          <Badge variant="destructive">{total.toLocaleString()} active</Badge>
        }
        size="sm"
        actions={
              ...
              <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                ...
                <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Narrow down by device and time range.
                    </DialogDescription>
                  </DialogHeader>
