...
export default function HHBridgedReportPage() {
  ...
  const fetchReports = useCallback(async () => {
    ...
    try {
      const res: any = await ReportService.getBridgeReports(filters);

      // Handle the actual backend response structure: { success, msg, data: { reports, pagination } }
      ...
      toast.error("Failed to load HH Bridge reports");
      console.error(err);
      setReports([]);
      setTotal(0);
    } finally {
  ...
  const handleDownload = (url: string, date: string) => {
    ...
    a.href = url;
    a.download = `hh-bridge-report-${date}.csv`; // Nice filename
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
  };

  const columns: ColumnDef<BridgeReport>[] = [
    {
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title="HH Bridge Reports"
        description="Daily generated Half-Hourly Bridge reports available for download"
                  ...
                  <TableCell colSpan={4} className="h-64 text-center">
                    <Empty>
                      <EmptyHeader>
                        ...
                        <EmptyDescription>
                          No HH Bridge reports available for the selected date range
                        </EmptyDescription>
                      </EmptyHeader>
                    </Empty>
                  </TableCell>
