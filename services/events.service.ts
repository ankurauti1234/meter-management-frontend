...
class EventsService {
  ...
  async getEventsByType(type: number, filters: Omit<EventFilters, "type"> = {}): Promise<PaginatedEvents> {
    const params = new URLSearchParams();
    if (filters.device_id) params.append("device_id", filters.device_id);
    if (filters.start_time) params.append("start_time", String(filters.start_time));
    if (filters.end_time) params.append("end_time", String(filters.end_time));
  ...
  async getAlertsByDevice(device_id: string, filters: Omit<EventFilters, "device_id"> = {}): Promise<PaginatedEvents> {
    const params = new URLSearchParams();
    if (filters.start_time) params.append("start_time", String(filters.start_time));
    if (filters.end_time) params.append("end_time", String(filters.end_time));
    if (filters.page) params.append("page", String(filters.page));
    ...
    const res = await api.get(`/events/alerts/device/${device_id}?${params.toString()}`);
    return res.data.data;
  }
